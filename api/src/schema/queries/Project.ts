...
export const GET_ALL_PROJECTS = {
    ...
    resolve() {
        return Project.find({ relations: ["client"] });
    }
}
...
export const FIND_PROJECT_BY_PROJECT_ID = {
    ...
    async resolve(parent: any, args: any) {
        const { id } = args
        const project = await Project.findOne({ id: id }, { relations: ["client"] })
...
export const FIND_PROJECTS_BY_CLIENT_ID = {
    ...
    async resolve(parent: any, args: any) {
        const { id } = args
        const client = await Client.findOne({ id: id }, { relations: ["client"] })
